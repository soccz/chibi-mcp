...
| **PyPI 배포 갱신** (이미 배포됨, 낡음) | `[chibi-mcp 이미 배포·소유 — 1.1.0(2026-05-28)까지 15릴리스. 그러나 PyPI 최신이 1.1.0이라 공개 설명에 옛 음식-특화 브랜딩이 라이브 노출 중. 레포는 1.4.39로 정화 완료. 현재 깨끗한 버전을 발행하면 공개 설명도 갱신됨(브랜드 위반 해소). CI Trusted Publishing 경로 완비, 실제 발행은 계정 작업]` |
