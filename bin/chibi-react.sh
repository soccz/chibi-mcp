...
# picks a contextual phrase, fires it via `chibi-say`. PostToolUse always records
# one tool call so rhythms/tickets progress; speech bubbles stay throttled.
#
...
if [ "$stage" = "pre" ]; then
  ...
  if [ -n "$phrase" ]; then
    chibi-say --tool-call "$phrase" >/dev/null 2>&1 || true
  else
    chibi-say --tool-call >/dev/null 2>&1 || true
  fi
fi
